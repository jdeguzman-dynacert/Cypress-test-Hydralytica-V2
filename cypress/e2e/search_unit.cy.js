...
describe('Hydralytica v2 - Search Unit', () => {
    const timestamp = Date.now();
    const uniqueUnitName = `My Cypress Unit`;
    const existingUnit = 'My Cypress Unit'; // adjust if you know one exists
    const nonExistentUnit = `NotARealUnit-${timestamp}`; // guaranteed to not exist
    ...
    // ------------------ SEARCH -------------------
    it('should search for an unit by name (new Unit)', () => {
        // Open field dropdown
        ...
        // Type the unique Unit name
        cy.get('input[name="value"]').clear().type(uniqueUnitName);

        // Click search
        cy.contains('button', 'Search').click();

        // Verify the Unit appears in results
        cy.contains(uniqueUnitName, { timeout: 5000 }).should('be.visible');
    });

    it('should reset filters', () => {
        cy.get('input[placeholder="Criteria"]').type(uniqueUnitName);
    ...
    it('should find an existing unit by name', () => {
        cy.get('button[aria-label="Select field"]').eq(0).click({ force: true });
        cy.contains('Name').click({ force: true });

        cy.get('input[name="value"]').clear().type(existingUnit);
        cy.get('button[aria-label="Search"]').click();

        cy.contains(existingUnit, { timeout: 10000 }).should('be.visible');
    });

    it('should show no results for a non-existent unit', () => {
        cy.get('button[aria-label="Select field"]').eq(0).click({ force: true });
        ...
        // enter a fake Unit
        cy.get('input[name="value"]').clear().type(nonExistentUnit);
        cy.get('button[aria-label="Search"]').click();
